...
module Sheet = {
  ...
  let make = (~constants: constants) => {
	  ...
	  let getMargin: string => (int, int) = s => {
        ...
        switch (elem) {
          | Some(e) => {
              ...
              let top  = String.split_on_char('p', topMargin) |> List.hd |> float_of_string |> int_of_float;
              let left = String.split_on_char('p', leftMargin) |> List.hd |> float_of_string |> int_of_float;
              (top, left)
            }
          | _ => (0, 0)
	  }
    ...
    <div id="sheet" key="sheet" style=sheetStyle onClick=handleClick>
      {List.init(constants.measuresPerLine*constants.nLines, _ => <div style=measureStyle> {React.string("test")} </div>) |> Array.of_list |> React.array}
      {notes
       |> List.map(note =>
            <div
